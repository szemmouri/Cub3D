...
{
    // Convert parsing coordinates to rendering coordinates
    game->player.x = game->player.pos_x * BLOCK;
    game->player.y = game->player.pos_y * BLOCK;
    
    // Convert direction to angle
    if (game->player.dir == 'N')
        game->player.angle = PI * 1.5; // 270 degrees
    else if (game->player.dir == 'S')
        game->player.angle = PI * 0.5; // 90 degrees
    else if (game->player.dir == 'E')
        game->player.angle = 0;        // 0 degrees
    else if (game->player.dir == 'W')
        game->player.angle = PI;       // 180 degrees
...
int main(int ac, char **av)
{
    ...
    // Set up MLX hooks and start the game loop
    mlx_hook(game.win, 2, 1L << 0, key_press, &game);
    mlx_hook(game.win, 3, 1L << 1, key_release, &game);
    mlx_hook(game.win, 17, 1L << 17, exit_game, &game); // Window close button
